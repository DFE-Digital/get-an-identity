locals {
  ...
  auth_server_env_vars = merge(
    ...
    local.auth_server_notify_domain_allow_list_env_vars,
    {
      ...
      DqtEvidence__StorageConnectionString         = "DefaultEndpointsProtocol=https;AccountName=${azurerm_storage_account.data-protection.name};AccountKey=${azurerm_storage_account.data-protection.primary_access_key}",
      DqtEvidence__StorageContainerName            = azurerm_storage_container.dqt_evidence.name,
